...
app.MapGet("/", HandleCount(() => GuidGenerator.Version4.NewGuid()));

app.MapGet("/v1", HandleCount(() => GuidGenerator.Version1.NewGuid()));
app.MapGet("/v1r", HandleCount(() => GuidGenerator.Version1R.NewGuid()));
...
app.MapGet("/v2/org/{siteId}", (uint siteId) => GuidGenerator.Version2.NewGuid(DceSecurityDomain.Org, (int)siteId));
app.MapGet("/v2/{domain}/{localId}", (byte domain, uint localId) => GuidGenerator.Version2.NewGuid((DceSecurityDomain)domain, (int)localId));

app.MapGet("/v3/{ns}/{name}", (string ns, string name) => GuidGenerator.Version3.NewGuid(ParseGuidNs(ns), name));
app.MapPost("/v3/{ns}", (string ns, [FromBody] string name) => GuidGenerator.Version3.NewGuid(ParseGuidNs(ns), ParseBase64(name)));

app.MapGet("/v4", HandleCount(() => GuidGenerator.Version4.NewGuid()));
...
app.MapGet("/v7", HandleCount(() => GuidGenerator.Version7.NewGuid()));

app.MapGet("/v8", HandleCount(() => GuidGenerator.Version8.NewGuid()));
app.MapGet("/v8n/{hash}/{ns}/{name}", (string hash, string ns, string name) => ParseHashName(hash).NewGuid(ParseGuidNs(ns), name));
app.MapPost("/v8n/{hash}/{ns}", (string hash, string ns, [FromBody] string name) => ParseHashName(hash).NewGuid(ParseGuidNs(ns), ParseBase64(name)));
#endif
